...
// a 'std::vectore' where 'T' is the '::value_type' of a container 'C'
template <typename C>
using to_vector_t = std::vector<typename std::decay<C>::value_type>;
...
template <typename C>
using container_reference_value_t = typename std::decay<C>::value_type&;
