...
std::function<int(int)> give_closure(int a, int b, int c) {
    // use copy capture to avoid dangling references or possible unsynchronized
    // thread access
    return [=](int d) {
...
A function generated by give_closure() will work even if passed to another thread!
