...
The `typename`/`class` types defined in a `template` are not known until the `template` is called in code. This means a template has the ability to *potentially* represent valid code for different types. 
